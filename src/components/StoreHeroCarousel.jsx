...
import { ChevronLeft, ChevronRight, CheckCircle, Clock, Copy, ExternalLink, TrendingUp } from 'lucide-react';
import { useCouponsByStore, useStores } from '../hooks/useAPI';

const StoreHeroCarousel = ({ storeId }) => {
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isAutoPlaying, setIsAutoPlaying] = useState(true);
  const [copiedCode, setCopiedCode] = useState(null);
  
  // API data
  const { data: storeCoupons = [], isLoading: couponsLoading } = useCouponsByStore(storeId);
  const { data: stores = [] } = useStores();
  
  // Get featured coupons for this store (trending ones)
  const featuredCoupons = React.useMemo(() => {
    return storeCoupons
      .filter(coupon => coupon.usage_count > 500) // Popular coupons
      .sort((a, b) => b.usage_count - a.usage_count)
      .slice(0, 3); // Max 3 slides for store page
  }, [storeCoupons]);
  ...
  if (couponsLoading) {
    return (
      <div className="w-full h-80 md:h-96 lg:h-[450px] bg-gradient-hero rounded-2xl flex items-center justify-center mb-8">
    ...
    return (
      <div className="w-full h-80 md:h-96 lg:h-[450px] bg-gradient-hero rounded-2xl flex items-center justify-center mb-8">
        <div className="text-center text-white">
          <h2 className="text-2xl font-bold mb-2">No Trending Deals</h2>
  ...
  return (
    <div 
      className="relative w-full h-80 md:h-96 lg:h-[450px] rounded-2xl overflow-hidden group mb-8"
